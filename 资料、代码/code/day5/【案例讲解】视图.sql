...
CREATE ALGORITHM = UNDEFINED DEFINER = `root`@`::1` SQL SECURITY DEFINER VIEW `myv3` AS SELECT
...
AS
SELECT * FROM departments WHERE department_id in (SELECT department_id FROM employees GROUP BY department_id HAVING MAX(salary) > 12000);
SELECT * FROM emp_v2;
...
SELECT * FROM myv1;
SELECT * FROM employees; (增删改在 原始表里居然也会有作用)
...
#具备以下特点的视图不允许更新(一般情况下, 视图不使用更新功能)
